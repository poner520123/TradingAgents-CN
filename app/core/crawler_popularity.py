...
class PopularityRankingCrawler:
    """人气排行爬虫"""
    ...
    def __init__(self):
        ...
        self.collection = self.db.popularity_ranking
    ...
    def _init_db(self):
        ...
        try:
            ...
            # Index for sorting by popularity score
            self.collection.create_index([("popularity_score", -1)])
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if retries == max_retries:
                        logger.warning(f"Failed to fetch popularity ranking page after {max_retries} retries: {response.status_code}")
                ...
                if retries == max_retries:
                    logger.error(f"Error fetching popularity ranking page after {max_retries} retries: {type(e).__name__}: {str(e)}")
                self.consecutive_errors += 1
                    ...
                    try:
                        ...
                        try:
                            if change_percent:
                                ...
                                popularity_score += abs(change) * 10
                        except:
                            pass
    ...
    def crawl_popularity_ranking(self):
        """爬取人气排行数据"""
        total_saved = 0
    ...
    def check_crawl_status(self):
        ...
        if count == 0:
            return {"status": "no_data", "message": "无人气排行数据"}
        
        return {"status": "healthy", "message": "爬虫运行正常", "last_crawl": self.last_crawl_time, "data_count": count}

# 全局人气排行爬虫实例
popularity_ranking_crawler = PopularityRankingCrawler()
