<template>
  <div class="capital-flow-ranking">
    <el-card>
      <template #header>
        <div class="card-header">
          <div class="header-left">
            <span class="title">资金流向排行榜</span>
...
import { getCapitalFlowData, type CapitalFlowItem } from '@/api/astock'
import { favoritesApi } from '@/api/favorites'

const router = useRouter()
const loading = ref(false)
const tableData = ref<CapitalFlowItem[]>([])
...
const analyzeStock = (row: CapitalFlowItem) => {
  // 使用股票代码进行分析
  router.push({
...
const addToFavorites = async (row: CapitalFlowItem) => {
  if (!row.name) {
